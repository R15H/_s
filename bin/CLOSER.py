...
def calculate_by_sample_cost(data,r):
                def get_all_instructions():
                    ...
                    d['real_slowdown'] =  (gcres80['currentCycle'][-1] - gcres['currentCycle'][-1])  / gcres['currentCycle'][-1]
                    # obtain nr of entries
                    nr_of_insts = len(inst['stallTime'])
                    d['cost_inst_LLCmiss'] = nr_of_insts / gcres['currentCycle'][-1]
                    d['cost_inst_stall_time'] = np.sum(i['stallTime']) / gcres['currentCycle'][-1]
                    d['cost_inst_L3stall_time'] = np.sum(i['L3stallTime']) / gcres['currentCycle'][-1]
                    d['cost_inst_L3MLP'] = np.sum(i['L3stallCyclesMLPLoad']) / gcres['currentCycle'][-1]
                    d['cost_inst_MLP'] = np.sum(i['stallCyclesMLPLoad']) / gcres['currentCycle'][-1]

                    d['cost_inst_L3stall/total'] = np.sum(i['L3stallTime']/i['totalTime']) / gcres['currentCycle'][-1]

                    d['cost_inst_3MLP/3stall'] = np.sum(i['L3stallCyclesMLPLoad']/i['L3stallTime']) / gcres['currentCycle'][-1]
                    d['cost_inst_MLP/stall'] = np.sum(i['stallCyclesMLPLoad']/i['stallTime']) / gcres['currentCycle'][-1]
                    d['cost_inst_MLP*total/stall'] = np.sum(i['stallCyclesMLPLoad']*i['totalTime']/i['stallTime']) / gcres['currentCycle'][-1]
                    d['cost_inst_MLP/total'] = np.sum(i['stallCyclesMLPLoad']/i['totalTime']) / gcres['currentCycle'][-1]
...
weights = ("cost_inst_MLP", "cost_inst_MLP/stall", "cost_inst_LLCmiss",
...
for w in weights:
    ...
    plt.scatter(all_data_real, all_data_weight, alpha=0.3)

    plt.xlabel('Real')
    plt.ylabel('Predicted')
    plt.legend()
    plt.savefig(f'{FIGS_FOLDER}/gen/insts/{w.replace("/", "D").replace("%", "P")}.png')
    plt.close()
