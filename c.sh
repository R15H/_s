#!/bin/bash
#
# c.sh — older variant of controller.sh (~500 fewer lines)
#
# This file shares its first ~120 lines with controller.sh and contains the
# same core functions (max_memory, prepare, rungem5_with_report, wait_for_space,
# synthethics_all, make_table_readable, glatTable, get_gem5_cmd_line).
#
# Differences from controller.sh:
#   - wait_for_space threshold is 8 concurrent jobs (vs 13 in controller.sh)
#   - Does not include short_executions() or current_runs()
#
# Prefer controller.sh for new work.  See controller.sh for full documentation.
...
#        set -x
    ...
    {
                now_time=$(date +%s)
                    $GEM5 $CONFIG "$BINARY" --latency-increase $increase --bargs "$ARGS" --cpu-start KVM --dramsize=${DRAM}GiB --bstdin "$STDIN" --skip_start_duration $SKIP_SECONDS --exec_timeout $TIMEOUT_SECONDS 2>&1 | tee -a  $nas/osdi26/results_gem5/err_$benchset\_$benchnr\_$increase\_  &
